...
Web endpoints: `POST /api/chat` (alias `POST /api/query`; accepts
`message`/`question` and `session_id`/`conversation_id`),
`POST /api/reset`, `GET /api/status`, `GET /api/examples`,
`GET /api/health` (alias `/health`). Sessions are server-side:
follow-up answers ("CGAIN") and clarification chips work across
requests; a session id is issued on the first message and stored in
the browser. Frontend features: copy button on every assistant output,
dark/light theme (persisted, defaults to system preference), Enter to
send / Shift+Enter for a newline, loading and error states, New chat
reset, responsive layout with a mobile top bar. The backend URL is
...
Explicit elliptical follow-ups ("What about the best?", "And at
2600?", "Show the same for node 45.") inherit the stable dimensions
from the previous answered intent — including the request, so
frequency-only follow-ups work; params are never inherited (the
assistant asks for a parameter the follow-up does not restate).
`/reset` (or the web New-chat button) clears everything.
...
- **`DEPLOYMENT.md`** — run modes (single process, dev, Docker),
  environment variables, key management, dataset placement, security
  notes and deployment blockers.
- **`PROJECT_CONTEXT.md`** — permanent architecture and decisions for
  future contributors and coding agents.
- **`OFFICE_REVIEW.md`** — open business-rule items awaiting office
  confirmation.
